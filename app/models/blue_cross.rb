...
class BlueCross < ApplicationRecord
    ...
    def self.get_animal_name_from_front_page(animal_data)
        animal_data.css(".item__title").text
    end

    def self.get_info_from_front_page(animal_data)
        @animal_url = "https://www.bluecross.org.uk#{animal_data["href"]}"
        @name = self.get_animal_name_from_front_page(animal_data)
    ...
    def self.get_data_from_individual_pet_page
        ...
        @colour = @page_data.at_css(".pet-details_species").text.gsub(/\n/, "").strip.gsub(/,/, "").split(" ").select{|word| Colours.all.include?(word.downcase)}.join(", ")
    end
    ...
    def self.pet_hash(animal_data)
        ...
        return {name: @name, breed: @breed, location: @location, description: @description, age: @age, link: @animal_url, photo: @photo, colour: @colour, home_id: self.get_home_id, species: ""}
    end

    def self.get_array_of_animals(animal_type)
        @animals = self.get_page(animal_type).css(".item__link")
        
        #reject reserved animals
        @animals = @animals.select{|animal| animal.css(".banner--reserved").length == 0}

        #reject animals which need to be adopted in pairs
        @animals.reject!{|animal| self.get_animal_name_from_front_page(animal).include?("and ")}
        @animals.reject!{|animal| self.get_animal_name_from_front_page(animal).include?("&")}
    end
